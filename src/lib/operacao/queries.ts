...
export async function listOperacaoTree(): Promise<OperacaoParte[]> {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id,slug,numero,titulo,descricao,ordem,ativo,paginas:operacao_paginas(id,parte_id,slug,titulo,resumo,ordem)",
    )
    .eq("operacao_paginas.ativo", true)
    .order("ordem")
    .order("ordem", { referencedTable: "operacao_paginas" });
  if (error)
    throw new Error(`Não foi possível carregar o manual: ${error.message}`);
...
export async function getOperacaoPagina(parteSlug: string, paginaSlug: string) {
  ...
  const { data, error } = await supabase
    ...
    .select(
      "id,parte_id,slug,titulo,resumo,conteudo,ordem,atualizado_por,updated_at,parte:operacao_partes!inner(id,slug,numero,titulo),autor:users!operacao_paginas_atualizado_por_fkey(nome),faqs:operacao_faqs(id,pagina_id,pergunta,resposta,ordem)",
    )
    .eq("ativo", true)
